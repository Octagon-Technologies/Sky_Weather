...
import com.octagontechnologies.sky_weather.domain.model.daily.DailyForecast
...
//    private val singleForecastJsonAdapter = moshi.adapter(SingleForecast::class.java)
    ...
    private val favouriteLocationJsonAdapter = moshi.adapter<LocalFavouriteLocation>(
        LocalFavouriteLocation::class.java)
    ...
    @TypeConverter
    fun stringToSingleForecast(data: String): SingleForecast? = singleForecastAdapter.fromJson(data)

    @TypeConverter
    fun singleForecastToString(currentWeatherDataClass: SingleForecast): String = singleForecastAdapter.toJson(currentWeatherDataClass)

    @TypeConverter
    fun stringToLunarForecast(data: String): Lunar? = lunarJsonAdapter.fromJson(data)

    @TypeConverter
    fun lunarForecastToString(lunarForecast: Lunar?): String = lunarJsonAdapter.toJson(lunarForecast)

    @TypeConverter
    fun stringToLocation(data: String): Location? = locationJsonAdapter.fromJson(data)

    @TypeConverter
    fun locationToString(location: Location?): String = locationJsonAdapter.toJson(location)

    @TypeConverter
    fun stringToRecentLocation(data: String): LocalRecentLocation? = recentLocationJsonAdapter.fromJson(data)

    @TypeConverter
    fun recentLocationToString(recentLocation: LocalRecentLocation?): String = recentLocationJsonAdapter.toJson(recentLocation)

    @TypeConverter
    fun stringToFavouriteLocation(data: String): LocalFavouriteLocation? = favouriteLocationJsonAdapter.fromJson(data)
    ...
    @TypeConverter
    fun stringToListOfHourlyForecast(data: String): List<SingleForecast>? = hourlyForecastJsonAdapter.fromJson(data)
    ...
    @TypeConverter
    fun stringToListOfEachDailyForecast(data: String): List<DailyForecast>? = dailyForecastJsonAdapter.fromJson(data)
